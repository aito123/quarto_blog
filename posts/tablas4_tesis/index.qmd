...
base1 %>% 
  ...
  geom_text(data = ~subset(., p207 %in% "Mujer"), aes(y=porc, label = label), position = position_stack(vjust = 0.6), size = 2.9) +
  geom_bar(data = ~subset(., p207 %in% "Hombre"), aes(y=-porc), stat="identity") +
  geom_text(data = ~subset(., p207 %in% "Hombre"), aes(y=-porc, label = label), position = position_stack(vjust = 0.4), size = 2.9) +
  scale_y_continuous(
    breaks = seq(-30, 30, 15),
